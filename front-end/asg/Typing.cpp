...
TranslationUnit *Typing::operator()(TranslationUnit *tu)
{
    for (auto &&i : tu->decls)
        self(i);
    return tu;
...
Expr *Typing::operator()(StringLiteral *obj)
{
    ArrayType arrTy;
    arrTy.len = obj->val.size() + 1;

    obj->type = mTypeCache(Type::Spec::kChar, Type::Qual{.const_ = true}, &arrTy);

    obj->cate = Expr::Cate::kRValue;
    return obj;
}

Expr *Typing::operator()(DeclRefExpr *obj)
{
    ASSERT(obj->decl);
...
Expr *Typing::operator()(ParenExpr *obj)
{
    ASSERT(obj->sub);
...
Expr *Typing::operator()(UnaryExpr *obj)
{
    ASSERT(obj->sub);
    Obj::Walked walked(obj);
...
Expr *Typing::operator()(BinaryExpr *obj)
{
...
Expr *Typing::operator()(CallExpr *obj)
{
    ASSERT(obj->head);
...
void Typing::operator()(ExprStmt *obj)
{
...
Expr *Typing::ensure_rvalue(Expr *exp)
{
...
Expr *Typing::promote_integer(Expr *exp, Type::Spec to)
{
...
Expr *Typing::assignment_cast(Expr *lft, Expr *rht)
{
    ...
    if (lft->type->texp != nullptr)
    {
        ...
        if (lft->type->qual.const_)
        {
            ...
            ccst->type = mTypeCache(rht->type->spec, Type::Qual{.const_ = true}, rht->type->texp);
            ccst->sub = rht;
            rht = ccst;
        }
...
Expr *Typing::infer_init(Expr *init, const Type *to)
{
...
std::pair<Expr *, std::size_t> Typing::infer_initlist(const std::vector<Expr *> &list, std::size_t begin,
                                                      const Type *to)
{
    if (to->texp == nullptr)
    {
