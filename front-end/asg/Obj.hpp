...
/// 错误断言，打印文件和行号，方便定位问题。
#define ASSERT(expr) ((expr) || (fprintf(stderr, "asserted at %s:%d\n", __FILE__, __LINE__), abort(), false))

/// 错误中断，打印文件和行号，方便定位问题。
#define ABORT() (fprintf(stderr, "aborted at %s:%d\n", __FILE__, __LINE__), abort())
...
struct alignas(ptrdiff_t) Obj
{
    ...
    template <typename T> T *dcst()
    {
        return dynamic_cast<T *>(this);
    }

    template <typename T> T *dcst() const
    {
        return dynamic_cast<T *>(this);
    }

    template <typename T> T *scst()
    {
        return static_cast<T *>(this);
    }

    template <typename T> T *scst() const
    {
        return static_cast<T *>(this);
    }

    template <typename T> T &rcst()
    {
        return *reinterpret_cast<T *>(this);
    }

    template <typename T> T &rcst() const
    {
        return *reinterpret_cast<T *>(this);
    }
    ...
    template <typename T> T *any_as()
    {
        return reinterpret_cast<T *>(any);
    }

    template <typename T> T *any_as() const
    {
        return reinterpret_cast<T *>(any);
    }

  private:
    Obj(Obj *next) : __next__(next)
    {
    }
...
struct Obj::Mgr : Obj
{
    ...
    template <typename T, typename... Args, typename = std::enable_if_t<std::is_convertible_v<T *, Obj *>>>
    T *make(Args... args)
    {
        auto obj = new T(args...);
