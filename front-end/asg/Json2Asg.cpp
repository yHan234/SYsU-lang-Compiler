...
NullStmt *Json2Asg::null_stmt(const llvm::json::Object &jobj)
{
    return make<NullStmt>();
}

BreakStmt *Json2Asg::break_stmt(const llvm::json::Object &jobj)
{
...
ContinueStmt *Json2Asg::continue_stmt(const llvm::json::Object &jobj)
{
...
ExprStmt *Json2Asg::expr_stmt(const llvm::json::Object &jobj)
{
...
const char *parse_base(const char *s, Type &v)
{
...
const char *parse_oct(const char *s, std::uint32_t &v)
{
    if (*s < '0' || *s > '9')
        return nullptr;
    std::uint32_t num = 0;
...
TypeExpr *turn_texp(TypeExpr *texp)
{
    if (texp->sub == nullptr)
        return texp;
...
const char *Json2Asg::parse_type(const char *s, const Type *&v)
{
    auto ty = make<Type>();
    s = parse_base(s, *ty);
    if (!s)
...
const char *Json2Asg::parse_texp(const char *s, TypeExpr *&v)
{
    return parse_texp_2(s, v);
}

const char *Json2Asg::parse_texp_0(const char *s, TypeExpr *&v)
{
RULE_1:
    if (auto p = match(s, "["))
    {
...
const char *Json2Asg::parse_texp_1(const char *s, TypeExpr *&v)
{
    TypeExpr *texp;
...
const char *Json2Asg::parse_texp_2(const char *s, TypeExpr *&v)
{
RULE_1:
    if (auto p = match(s, "*"))
    {
...
const char *Json2Asg::parse_args(const char *s, std::vector<const Type *> &v)
{
    const Type *ty;
